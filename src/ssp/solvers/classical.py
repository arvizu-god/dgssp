...
reference / baseline to compare against the quantum Draper–Grover and QPE
solvers.
"""
...
class DPSSPSolver(BaseClassicalSSPSolver):
    ...
    This solver implements essentially the same logic as the original
    `ClassicalSSP.dynamical_programming` method you provided, but:

    - Works on a SubsetSumInstance object instead of a raw (A, t) pair.
    - Tracks *indices* of items, so solutions can be mapped back to
      SSPSolution objects via the instance.
    - Returns a DPResult with best_solution, all_solutions, etc.
